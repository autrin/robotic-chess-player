...
REQUIRE_MOTION_APPROVAL = False
REQUIRE_MOTION_VISUALIZATION = False
...
def cleanup():
    global robot_running, robot_thread
    robot_running = False
...
setup_environment(cleanup_callback=cleanup)


def capture_board_state(camera, detector_obj):
    print("Reading frame...")
    ...
    for cluster in clusters:
        if cluster.is_piece_tag():
            ...
            idx: Optional[int] = cluster.get_vert_idx_if_corner()
            if idx is not None:
...
def prompt_for_move(game_state, camera, detector_obj):
    while True:
        input("Press Enter after move has been made...")
        img, solver, board_bins, certainty_grid = capture_board_state(camera, detector_obj)
        ...
        found_move = GameState.get_move_diff(game_state, board)
        if found_move:
            return found_move, board
        print("No move or illegal move detected. Adjust board and try again.")


def verify_move(expected_move, game_state, camera, detector_obj):
    while True:
        move_check, _ = prompt_for_move(game_state, camera, detector_obj)
...
def robot_thread_function(robot_p: ChessMovementController):
...
def set_robot_move(robot_move, is_captured: bool, is_en_passant: bool = False):
    ...
    ai_move = (robot_move, is_captured, is_en_passant)
    move_executed.clear()
...
orchestrator: Orchestrator = Orchestrator(
    skeleton=skeleton,
    require_viz=REQUIRE_MOTION_VISUALIZATION,
    require_approval=REQUIRE_MOTION_APPROVAL
)
...
try:
    ...
    if test_mode:
        ...
        if engine_is_white:
            ...
            is_capture_m, is_en_passant_m = GameState.classify_move(move, before)
            success = set_robot_move(move, is_capture_m, is_en_passant_m)
            if success:
        ...
        while not game.board.is_game_over() and not rospy.is_shutdown():
            try:
                ...
                if game.offer_move(human_move):
                    ...
                    is_capture_m, is_en_passant_m = GameState.classify_move(human_move, before)
                    if set_robot_move(human_move, is_capture_m, is_en_passant_m):
                        game.print_board()
                        print(f"FEN: {game.get_fen()}")
                        print("Stockfish:", engine.get_eval_score())
                        print("-" * 60)
                ...
                if game.board.turn == (chess.WHITE if engine_is_white else chess.BLACK):
                    ...
                    if game.offer_move(engine_move):
                        ...
                        is_capture_m, is_en_passant_m = GameState.classify_move(engine_move, before)
                        if set_robot_move(engine_move, is_capture_m, is_en_passant_m):
                            game.print_board()
                            print(f"FEN: {game.get_fen()}")
                            print("Stockfish:", engine.get_eval_score())
                            print("-" * 60)
    ...
    # OPTION 2: FULL MODE (vision-based)
        ...
        if engine_is_white:
            ...
            if REQUIRE_VERIFY_ENGINE_MOVE:
                verify_move(move, game, cam, detector)
            else:
                print(f"Skipping engine verification...")
