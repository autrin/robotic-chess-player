...
from jh1.utils.mathematics.affine import se3_rigid_mat4
...
def forward_kinematics(q):
    ...
    fk_mat = fk_mat @ se3_rigid_mat4(BASE_ROT_R, np.zeros(3))

    # 2) Shoulder link offset
    fk_mat = fk_mat @ se3_rigid_mat4(np.eye(3), _UR10E_ARMATURE_D1)
    ...
    fk_mat = fk_mat @ se3_rigid_mat4(R_Y_90, _UR10E_ARMATURE_SHOULDER_OFFSET)
    positions.append(fk_mat[:3, 3].copy())
    ...
    # 6) Main forearm link
    fk_mat = fk_mat @ se3_rigid_mat4(np.eye(3), _UR10E_ARMATURE_A2)
    positions.append(fk_mat[:3, 3].copy())

    # 7) Elbow-offset bone
    fk_mat = fk_mat @ se3_rigid_mat4(np.eye(3), _UR10E_ARMATURE_ELBOW_OFFSET)
    positions.append(fk_mat[:3, 3].copy())
    ...
    fk_mat = fk_mat @ se3_rigid_mat4(R_Y_90, _UR10E_ARMATURE_A3)
    positions.append(fk_mat[:3, 3].copy())
    ...
    fk_mat = fk_mat @ se3_rigid_mat4(np.eye(3), _UR10E_ARMATURE_D4)
    positions.append(fk_mat[:3, 3].copy())
    ...
    fk_mat = fk_mat @ se3_rigid_mat4(np.eye(3), _UR10E_ARMATURE_D5)
    positions.append(fk_mat[:3, 3].copy())
    ...
    fk_mat = fk_mat @ se3_rigid_mat4(np.eye(3), _UR10E_ARMATURE_EE_OFFSET)
    positions.append(fk_mat[:3, 3].copy())
